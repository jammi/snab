...
const texts = require('snab/cli/texts').restart;
const argvParser = require('snab/util/argv-parser');
const {err, log, warn} = require('snab/util/log')('snab:cmd:restart');
  ...
  return {
    ...
    run: () => {
      ...
      servicePaths.forEach(servicePath =>
        PluginLoader(servicePath, api, ['restart'])
          .loadAll()
          .then(plugins =>
            Promise.all(
              plugins.map(plugin =>
                Promise.resolve(plugin.restart())
              )
            )
          )
          .catch(error =>
